...
function RevisionList({ items, emptyTitle, emptyDescription }) {
  ...
  return (
    <div className="grid gap-4 xl:grid-cols-2">
      {items.map((revision) => (
        <Card key={revision._id} className="border-white/10 bg-white/5">
          <CardContent className="space-y-4 p-5">
            <div className="flex items-start justify-between gap-4">
              <div>
                <p className="text-xs text-muted-foreground">Due {formatDateTime(revision.dueDate)}</p>
                <h3 className="mt-2 text-lg font-semibold">{revision.questionId?.title || 'Question'}</h3>
              </div>
              <Badge variant={getStatusTone(revision.status)}>{revision.status}</Badge>
            </div>
            <p className="text-sm text-muted-foreground">Platform: {revision.questionId?.platform || '—'} · Difficulty: {revision.questionId?.difficulty || '—'}</p>
            ...
            <p className="text-xs text-muted-foreground">Those actions are intentionally disabled until corresponding backend endpoints exist.</p>
          </CardContent>
        </Card>
      ))}
    </div>
        ...
        <Card className="border-white/10 bg-white/5">
          <CardContent className="space-y-4 p-6 sm:p-8">
            ...
            <div className="grid gap-3 sm:grid-cols-3">
              ...
              <div className="rounded-2xl border border-white/10 bg-white/5 p-4"><p className="text-xs text-muted-foreground">Completed</p><p className="mt-2 text-2xl font-semibold text-emerald-300">{completedQuery.data?.length || 0}</p></div>
            </div>
          </CardContent>
        </Card>
        <Card className="border-white/10 bg-white/5">
