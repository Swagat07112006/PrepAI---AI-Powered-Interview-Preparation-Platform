    ...
    <PageShell className="space-y-6">
      ...
      {notes.length ? (
        <div className="grid gap-4 xl:grid-cols-2">
          {notes.map((note) => (
            <Card key={note._id} className="border-white/10 bg-white/5">
              <CardContent className="space-y-4 p-5">
                <div className="flex items-start justify-between gap-4">
                  <div className="min-w-0">
                    <p className="text-xs text-muted-foreground">{formatDateTime(note.createdAt)}</p>
                    <h2 className="mt-2 text-xl font-semibold leading-tight">{note.title}</h2>
                  </div>
                  <DropdownMenu>
                ...
                <div className="flex items-center justify-between gap-3">
                  ...
                  <Button variant="secondary" size="sm" asChild><Link to={`/notes/${note._id}`}>Open note</Link></Button>
                </div>
              </CardContent>
            </Card>
          ))}
